...
class InputFeatures(object):
    def __init__(self,
        ...
        self.choices_features = [
            ...
            for _, input_ids, segment_ids in choices_features
        ]
        self.label = label
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for example_index, example in enumerate(examples):
        ...
        for ending_index, ending in enumerate(example.endings):
            ...
            # "- 3"
            _truncate_seq_pair(context_tokens_choice, ending_tokens, max_seq_length - 4)

            tokens = ["</s>"] + context_tokens_choice + ["</s>", "</s>"] + ending_tokens + ["</s>"]
            segment_ids = [0] * (len(context_tokens_choice) + 2) + [1] * (len(ending_tokens) + 2)
            ...
            choices_features.append((tokens, input_ids, segment_ids))
        ...
        if example_index < 1:
            ...
            for choice_idx, (tokens, input_ids, segment_ids) in enumerate(choices_features):
...
def main():
    ...
    tokenizer = RobertaTokenizer.from_pretrained(args.bert_model, return_tensors='pt', do_lower_case=args.do_lower_case)
    ...
    if args.do_train:
        ...
        print(args.train_batch_size, args.gradient_accumulation_steps)
        num_train_steps = int(
    ...
    if args.do_train:
        ...
        for ep in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                outputs = model(input_ids=input_ids, attention_mask=segment_ids, labels=label_ids)
                loss = outputs[0]
                if n_gpu > 1:
            ...
            if global_step % 1000 == 0:
                ...
                for step, batch in enumerate(eval_dataloader):
                    ...
                    with torch.no_grad():
                        ...
                        outputs = model(input_ids=input_ids, attention_mask=segment_ids)
                        logits = outputs[1]
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for step, batch in enumerate(eval_dataloader):
            batch = tuple(t.to(device) for t in batch)
            input_ids, segment_ids, label_ids = batch
            input_ids, segment_ids, label_ids = input_ids.to(device), segment_ids.to(device), label_ids.to(device)
